...
template <typename MLdata, typename SampleType, typename InputType, typename OutputType>
float ANN_Training(MLdata* ptdata,const int* const topology, int topology_array_length,
    ...
    */
    ...
    if(bias){
        ...
        if(weight_bias_array_length!=length-topology[0]){
           if(max_iteration==1) cout<<"ERROR: wrong input weight"<<endl;
    ...
    for(int i=0;i<max_iteration;i++){
        ...
        if(validate){//!!Take care!!!! Output should store!!!
            vd_error_tmp=ANN_Training<MLdata,SampleType,InputType,OutputType>
            (ptvalidata,topology,num_layer,
        ...
        if(max_iteration>1){
            ...
            printf("error:%.6f\tIteration:%d\tValidate:%.6f",error,i,vd_error_tmp);
            printf("\r");
        }
...
float ANN_Training_bool(MLdata* ptdata,const int* const topology, int topology_array_length,
             float threshold_calculator(const MLdata* const,const float*),
    ...
    {
        return ANN_Training<MLdata,bool*,bool*,bool*>(ptdata,topology,topology_array_length,
             threshold_calculator,
             from_output_to_result,weight,weight_bias,result_store,
             threshold,
